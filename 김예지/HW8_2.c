...
int stringCompare(const char* s1, const char* s2) // 문자열이 같으면 0, 다르면 1 
{
	while (*s1 || *s2) {
		if (*s1 < *s2)
			return 1;
...
void stringCat(char* s1, const char* s2) // 문자열 접합 
{
	while (*s1)
		*s1++;
...
void stringChange(char* s, char ch, char newCh) // 문자열 변환 
{
	while (*s)
	{
		if (*s == ch)
